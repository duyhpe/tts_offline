...
class DriverProxy(object):
    ...
    def __init__(self, engine, driverName, debug):
        ...
        @Tham số engine: Tham chiếu đến trình điều khiển driver của máy
        @Kiểu dữ liệu engine: L{engine.Engine}
        @Tham số driverName: Tên của Driver được sử dụng/ hoặc
            None là để mặc định của platform
        @Kiểu dữ liệu driverName: str - Dữ liệu kiểu chuỗi (string)
        @Tham số debug: Cho phép/không cho phéo chạy gỡ lỗi
        @type debug: bool - Kiểu dữ liệu Logic True hoặc False
        '''
        ...
        name = 'tts_offline.drivers.%s' % driverName
